    ...
    for filename in filenames:
        if filename.endswith(".js") and not dirpath.endswith(".git") and "node_modules" not in dirpath:
            ...
            with open(full_path, "r", encoding="utf-8", errors="ignore") as f:
                content = f.read()
            # check for onInstalled or setUninstallURL
            if "onInstalled" in content or "setUninstallURL" in content or "uninstall" in content.lower():
                print(f"FOUND IN: {os.path.relpath(full_path, root_dir)}")
                for match in re.finditer(r'onInstalled|setUninstallURL|uninstall', content, re.IGNORECASE):
