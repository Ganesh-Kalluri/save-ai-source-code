...
# 2. Patch chunks/db-_VinwtYz.js and content-scripts/content.js in both dev and active directories
target_pattern = 'const formatMarkdownLineBreaks=text=>text?text.split("\\n").map(l=>{const esc=l.replace(/^(\\s*\\d+)\\./,"$1\\\\.");const t=esc.trimEnd();return t===\"\"?\"\":t+\"  \"}).join(\"\\n\"):\"\";'
replacement_pattern = 'const formatMarkdownLineBreaks=text=>text?text.trim():\"\";'
...
for path in paths_to_patch:
    if os.path.exists(path):
        ...
        if target_pattern in content:
            ...
            # Let's try matching with single backslashes in replacement just in case
            print(f"Pattern not found in {path}. Let's do a substring check.")
            # Print length of file to ensure it's not empty
            print(f"File size of {path}: {len(content)}")
    else:
