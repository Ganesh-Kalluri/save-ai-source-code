...
# Let's search for "SW" in JS files
    ...
    for filename in filenames:
        if filename.endswith(".js") and not dirpath.endswith(".git") and "node_modules" not in dirpath:
            ...
            with open(full_path, "r", encoding="utf-8", errors="ignore") as f:
                content = f.read()
            # find SW as a word or substring
            matches = list(re.finditer(r'\bSW\b', content))
            if matches:
                print(f"FOUND SW IN: {os.path.relpath(full_path, root_dir)}")
